-- ==============================================
-- 02_chargement_dwh.sql
-- Chargement initial (ELT) depuis le schéma operationnel vers dwh
-- ==============================================
...
WITH limites AS (
  ...
  FROM operationnel._commande
), dates AS (
  SELECT generate_series(dmin, dmax, interval '1 day')::date AS d FROM limites
)
INSERT INTO dwh.dim_date (date_key, full_date, year, quarter, month, month_name, week, day, is_weekend)
SELECT
...
-- Autres dimensions
INSERT INTO dwh.dim_client (codecli, societe, contact, pays, ville, region, codepostal)
SELECT DISTINCT codecli, societe, contact, pays, ville, region, codepostal
FROM operationnel._client
WHERE codecli IS NOT NULL
ON CONFLICT (codecli) DO NOTHING;

INSERT INTO dwh.dim_employe (noemp, nom, prenom, fonction, pays, ville, region)
SELECT DISTINCT noemp, nom, prenom, fonction, pays, ville, region
FROM operationnel._employe
WHERE noemp IS NOT NULL
ON CONFLICT (noemp) DO NOTHING;

INSERT INTO dwh.dim_fournisseur (nofourn, societe, pays, ville, region)
SELECT DISTINCT nofourn, societe, pays, ville, region
FROM operationnel._fournisseur
WHERE nofourn IS NOT NULL
ON CONFLICT (nofourn) DO NOTHING;

INSERT INTO dwh.dim_categorie (codecateg, nomcateg)
SELECT DISTINCT codecateg, nomcateg
FROM operationnel._categorie
WHERE codecateg IS NOT NULL
ON CONFLICT (codecateg) DO NOTHING;

INSERT INTO dwh.dim_transporteur (notran, nomtran)
SELECT DISTINCT notran, nomtran
FROM operationnel._transporteur
WHERE notran IS NOT NULL
ON CONFLICT (notran) DO NOTHING;

INSERT INTO dwh.dim_geo_livraison (pays, region, ville)
SELECT DISTINCT COALESCE(paysliv,'(NON RENSEIGNE)'), COALESCE(regionliv,'(NON RENSEIGNE)'), COALESCE(villeliv,'(NON RENSEIGNE)')
FROM operationnel._commande;

-- dim_produit (après categorie + fournisseur)
INSERT INTO dwh.dim_produit (refprod, nomprod, prix_catalogue, categorie_key, fournisseur_key)
SELECT
  p.refprod, p.nomprod, p.prixunit,
  dc.categorie_key, df.fournisseur_key
FROM operationnel._produit p
LEFT JOIN dwh.dim_categorie dc ON dc.codecateg = p.codecateg
LEFT JOIN dwh.dim_fournisseur df ON df.nofourn = p.nofourn
ON CONFLICT (refprod) DO NOTHING;

-- Fait ventes
INSERT INTO dwh.fact_ventes (
...
SELECT
  ...
  dgeo.geo_key
FROM operationnel._detailcommande dc
JOIN operationnel._commande co           ON co.nocom = dc.nocom
LEFT JOIN dwh.dim_client dcli            ON dcli.codecli = co.codecli
LEFT JOIN dwh.dim_employe demp           ON demp.noemp = co.noemp
LEFT JOIN dwh.dim_transporteur dtra      ON dtra.notran = co.notran
LEFT JOIN dwh.dim_geo_livraison dgeo     ON dgeo.pays = COALESCE(co.paysliv,'(NON RENSEIGNE)')
                                        AND dgeo.region = COALESCE(co.regionliv,'(NON RENSEIGNE)')
                                        AND dgeo.ville  = COALESCE(co.villeliv,'(NON RENSEIGNE)')
LEFT JOIN dwh.dim_produit dprod          ON dprod.refprod = dc.refprod
LEFT JOIN dwh.dim_categorie dcat         ON dcat.categorie_key = dprod.categorie_key
LEFT JOIN dwh.dim_fournisseur dfou       ON dfou.fournisseur_key = dprod.fournisseur_key
LEFT JOIN dwh.dim_date dd                ON dd.full_date = NULLIF(co.datecom,'')::date;

-- Fait livraisons
INSERT INTO dwh.fact_livraisons (
...
SELECT
  ...
  demp.employe_key,
  dcli.client_key
FROM operationnel._commande co
LEFT JOIN operationnel._detailcommande dc ON dc.nocom = co.nocom
LEFT JOIN dwh.dim_client dcli             ON dcli.codecli = co.codecli
LEFT JOIN dwh.dim_employe demp            ON demp.noemp = co.noemp
LEFT JOIN dwh.dim_transporteur dtra       ON dtra.notran = co.notran
LEFT JOIN dwh.dim_geo_livraison dgeo      ON dgeo.pays = COALESCE(co.paysliv,'(NON RENSEIGNE)')
                                         AND dgeo.region = COALESCE(co.regionliv,'(NON RENSEIGNE)')
                                         AND dgeo.ville  = COALESCE(co.villeliv,'(NON RENSEIGNE)')
LEFT JOIN dwh.dim_date dd_com ON dd_com.full_date = NULLIF(co.datecom,'')::date
LEFT JOIN dwh.dim_date dd_env ON dd_env.full_date = NULLIF(co.dateenv,'')::date
LEFT JOIN dwh.dim_date dd_obj ON dd_obj.full_date = NULLIF(co.dateobjliv,'')::date
